...
| 2026-07-10 | HTML `Cache-Control` in `deploy-blog.yml` changes from `max-age=60,s-maxage=300` to `max-age=60,s-maxage=604800` (browser unchanged, CDN raised to one week) | The short `s-maxage` existed as a safety net for the old, unreliable git-diff invalidation; now that the manifest mechanism triggers precise, immediate invalidation on every publish, the edge's natural TTL is a ceiling, not the primary freshness mechanism, so it can be long without risking staleness. `max-age` (the browser-facing value) stays short and unchanged, since CloudFront invalidation can never reach an already-cached browser copy — that value alone bounds worst-case staleness for a returning visitor. A stale `max-age` re-fetch is a CloudFront edge cache hit (no origin/S3 cost), so shortening it further has no cost benefit either way |
| 2026-07-11 | Bugfix: `routing.mjs`'s `alwaysInvalidatePaths` now targets `{homePath}index.html` instead of the bare `{homePath}` | Invalidations were firing but not actually clearing the cached home page: the edge CloudFront Function (`infra/lib/constructs/edge-function.ts`) rewrites directory-style URIs to append `index.html` **at viewer-request, before the cache lookup**, so the object CloudFront actually caches is keyed by `.../index.html`, not the bare `/{lang}/`. Deliberately not a `{homePath}*` wildcard — that would invalidate every page under the language on every single publish (`alwaysInvalidatePaths` runs unconditionally on every deploy), defeating the point of scoped invalidation. Same root cause fixed on the panel side for post pages — see `panel/spec.md`'s matching entry (post entries there use a wildcard suffix instead, since nothing else lives under a post's own path) |