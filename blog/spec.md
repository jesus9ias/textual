...
Two private S3 buckets, per the monorepo Domain Model / CI/CD contract:

- **`entries`** — one object per published post page (`/{...}/index.html`), long CloudFront TTL, invalidated only for the specific post(s) that changed.
- **`shell`** — home, `historico`, category/tag/author listings, `sitemap.xml`, `rss.xml`, `robots.txt`, shared CSS/JS/fonts. Short TTL or explicit invalidation, since it changes on every publish.

### CloudFront

One distribution, two behaviors (one origin per bucket), each with its own Origin Access Control (OAC) — see Security Contract in the monorepo spec. Behavior routing is by path prefix matching the entries route pattern (e.g. post URL shape) vs. everything else.

A single **CloudFront Function** is attached to the `shell` behavior's viewer-request event. It matches only the exact request URI `/` and returns a `301` to `/{defaultLang}/`; every other URI passes through unmodified. `{defaultLang}` is not read at runtime — it is interpolated into the function's source at CDK synth time from `DEFAULT_LANG` in `.env`, keeping the function free of any config lookup.
...
The whole stack is named from `STACK_NAME` in `blog/infra/.env`. That name is both the CloudFormation stack name and the prefix of every auto-generated physical resource name (buckets, etc. — none of which is hardcoded). So the same project can be deployed as several independent stacks — one `.env` per subdomain (or per fork, e.g. when running different templates) — each with its own domain, certificate, buckets, distribution, and DNS records, and each tagged `Project=<STACK_NAME>`. Different stacks must use different subdomains; nothing is shared between them.
...
5. Output: two path lists (entries, shell) consumed by the sync and invalidation steps.
...
Open item carried forward: the `entries`/`shell` CloudFront behavior path partition is provisional (Stage 1) and must be finalized together with the S3 sync split (Stage 4); see the relevant Decisions Log rows.
...
| 2026-07-06 | (Stage 4) The S3 sync splits post pages (`{lang}/{slug}/`) to the `entries` bucket and everything else to `shell`, listed by `scripts/list-entry-paths.mjs`; scoped invalidation is fed by `detect-changed-views.mjs` | Matches the two-bucket TTL split. The sync partition must stay aligned with the CloudFront behavior path patterns, which remain provisional until the entries/shell routing is finalized (Stage 1 open item) — flagged in the workflow and infra readme |
