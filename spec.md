...
│       ├── bin/textual.ts
│       ├── lib/
│       │   ├── textual-stack.ts      # hosted-zone reference + certificate + site bucket + CloudFront
│       │   └── constructs/
│       │       ├── static-site.ts            # 1× private S3 + CloudFront (1 OAC, 1 behavior)
│       │       └── edge-function.ts          # CloudFront Function: "/" → 301 "/{defaultLang}/" + directory index rewrite
│       ├── cdk.json
│       └── .env
│
├── panel/
...
| Hosting | 1× private S3 bucket + 1× CloudFront (1 behavior, 1 OAC). TTL differentiation is done with per-object `Cache-Control` at publish time, not a bucket split |
| Edge function | 1× CloudFront Function on the single behavior — (a) exact `/` → 301 `/{defaultLang}/` (`defaultLang` baked in at synth from `DEFAULT_LANG`), and (b) directory-index rewrite so clean URLs (`/es/`, `/en/how-black-holes-form`) resolve to the stored `…/index.html` |
...
- The bare domain root (`/`) is not a content page. It returns a `301` redirect to `/{defaultLang}/`, generated by the edge CloudFront Function (edge-only, no server) on the single site behavior, matching the exact path `/`. The redirect target is fixed and deterministic — it does not vary by `Accept-Language`, geography, or any per-request signal, so every visitor and every crawler is redirected identically. Deep links (`/es/mi-post`), static assets, and files like `/sitemap.xml` or `/robots.txt` are untouched by this rule.
...
- **`blog/infra` is never touched by CI.** It is deployed manually from local via `cdk deploy`. The GitHub Actions workflow requires no CloudFormation/CDK IAM permissions — only scoped S3 (the single site bucket) and CloudFront (create-invalidation) permissions.
...
- The S3 site bucket is fully private; access is granted exclusively to CloudFront via a dedicated Origin Access Control (OAC) scoped to that distribution.
- The GitHub Actions IAM role/user is scoped to `s3:PutObject`/`s3:DeleteObject` on the single site bucket and `cloudfront:CreateInvalidation` on the specific distribution — nothing broader, given the repository is public.
