 ...
 * `INVALIDATION_MANIFEST_PATH` points at the CloudFront invalidation manifest,
 * deliberately outside `blog/` (see panel/spec.md's Publish Invalidation
 * Manifest and blog/spec.md's Decisions Log).
 */
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): PanelConfig {
  ...
  const manifestPath = env.INVALIDATION_MANIFEST_PATH?.trim();
  const missing: string[] = [];
  if (!contentRoot) missing.push('BLOG_CONTENT_PATH');
  if (!configRoot) missing.push('BLOG_CONFIG_PATH');
  if (!manifestPath) missing.push('INVALIDATION_MANIFEST_PATH');
  if (missing.length > 0) {
  ...
  return {
    ...
    configRoot: configRoot as string,
